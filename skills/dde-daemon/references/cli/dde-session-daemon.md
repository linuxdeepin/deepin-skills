**用途**：DDE 会话守护进程，是 DDE 桌面会话中运行的后台核心服务。它负责管理各种功能模块的加载、启用和禁用，包括网络、蓝牙、声音、电源、账户、显示等数十个模块。通过模块化的架构设计，可以灵活地启用或禁用特定功能模块。支持日志级别控制、强制启动已禁用模块、列出模块及其依赖关系等。该守护进程在用户登录后由会话管理自动启动，是 DDE 桌面环境正常运行的基础服务。使用 Go flag 风格参数。
