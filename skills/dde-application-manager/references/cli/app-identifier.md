**用途**：应用身份识别工具，用于识别指定进程以何种身份（应用 ID）运行。它通过查询 dde-application-manager 的应用管理信息，将进程 PID 映射到对应的应用标识。该工具主要用于 DDE 的应用权限管理和安全审计场景，例如判断某个进程属于哪个应用、是否以沙箱方式运行等。通常由系统安全组件或调试工具调用。

- **所属包名**：`dde-application-manager`
- **安装路径**：`/usr/bin/app-identifier`
- **DDE 角色**：系统调用的辅助工具，用于应用身份识别和安全审计

**用法**：`app-identifier [options] [pid1,pid2,pid3 ...]`
