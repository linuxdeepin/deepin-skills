**用途**：SVG 转 PNG 转换工具，将 SVG 矢量图渲染为 PNG 位图。支持通过 `DSvgRenderer`（DTK 增强渲染器）或 `QSvgRenderer`（Qt 原生渲染器）进行渲染，支持指定输出尺寸（宽x高、仅宽、仅高）和缩放因子。该工具适用于 SVG 图标资源的位图化处理。

- **所属 DTK**：DTK 开发工具集
- **安装路径**：`/usr/bin/dtk6-svgc`
- **DDE 角色**：开发工具，用于 SVG 图片转 PNG

**用法**：`dtk6-svgc [options] <input> <output>`
