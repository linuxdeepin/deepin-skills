**用途**：DConfig 转 C++ 代码生成器，从 DConfig JSON 配置文件生成 C++ 头文件，将 DConfig 配置项封装为类型安全的 C++ 类。生成的类包含属性定义、默认值、值变更信号等，支持强制请求线程创建 DConfig 实例和禁止生成注释。该工具用于在编译期将 DConfig 配置项静态化，减少运行时 DBus 调用开销。

- **所属 DTK**：DTK 开发工具集
- **安装路径**：`/usr/bin/dconfig2cpp`
- **DDE 角色**：开发工具，用于 DConfig 配置项的 C++ 代码生成

**用法**：`dconfig2cpp [options] <json-file>`
