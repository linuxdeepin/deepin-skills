**用途**：DTK GUI 设置工具，用于读写 X11 窗口属性设置。它可以直接操作窗口的属性（如窗口标题、图标、透明度等），支持通过窗口 ID 或在屏幕上选择窗口来指定目标窗口。可以设置字符串、整数、颜色等类型的属性值，也支持移除指定属性。该工具主要用于 DTK 窗口属性的调试和自定义，适用于开发者和高级用户。

- **所属 DTK**：DTK 开发工具集
- **安装路径**：`/usr/bin/deepin-gui-settings`
- **DDE 角色**：开发调试工具，用于窗口属性读写和调试

**用法**：`deepin-gui-settings [options] [keys]`
