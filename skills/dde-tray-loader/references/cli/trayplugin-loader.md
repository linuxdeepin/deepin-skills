**用途**：DDE 托盘插件加载器，负责加载和管理系统托盘区域的插件。它根据指定的插件路径和组名，将托盘插件动态加载到 DDE 面板的托盘区域中。该工具通常由 dde-shell 或面板管理进程在会话启动时自动调用，一般不需要用户直接运行。支持按组名分组加载插件，适用于多面板或多区域托盘场景。

- **所属包名**：`dde-tray-loader`
- **安装路径**：`/usr/bin/trayplugin-loader`
- **DDE 角色**：系统调用的辅助工具，由 dde-shell 在会话启动时自动拉起

**用法**：`trayplugin-loader [options]`
