**用途**：DDE 重置密码对话框，用于在用户忘记密码时提供密码重置功能。它通过 PAM 认证机制验证用户身份后允许设置新密码。支持指定用户名和应用名参数，通常在登录界面（如 lightdm-deepin-greeter）中由用户点击"忘记密码"触发。该工具也可以在系统恢复或运维场景中手动调用。

- **所属包名**：`dde-session-ui`
- **安装路径**：`/usr/bin/reset-password-dialog`
- **DDE 角色**：用户使用的密码重置工具，登录界面中触发

**用法**：`reset-password-dialog [options]`
