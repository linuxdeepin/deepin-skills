**用途**：图片模糊处理工具，使用高斯模糊算法对图片进行模糊处理。它接受输入图片文件路径和输出文件路径，通过 `-sigma` 参数控制模糊强度（默认 20.0，值越大模糊程度越高）。该工具主要用于 DDE 界面中需要模糊背景效果的场景，例如锁屏壁纸的模糊处理、控制中心的毛玻璃效果背景等。通常由系统主题组件或壁纸管理模块调用。使用 Go flag 风格参数。

- **所属包名**：`dde-api`
- **安装路径**：`/usr/bin/image-blur`
- **DDE 角色**：系统调用的图片处理辅助工具

**用法**：`image-blur [options] <input> <output>`
