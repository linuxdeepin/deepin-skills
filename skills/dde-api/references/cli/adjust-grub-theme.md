**用途**：GRUB 主题调整工具，用于根据屏幕分辨率和语言环境自动调整 GRUB 启动菜单的主题显示。它会从主题输入目录读取原始 GRUB 主题文件，根据指定的屏幕宽高和语言参数进行适配处理，将结果输出到主题输出目录（默认 `/boot/grub/themes/`）。还支持设置背景图片和调整终端字体。该工具通常在系统安装、分辨率变更或主题更新时由系统自动调用，确保 GRUB 启动界面在不同硬件上正确显示。使用 Go flag 风格参数。
